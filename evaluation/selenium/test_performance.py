    ...
    def _lare_click(self, site_url_1, site_url_2):
        ...
        for caching in [{'DB': False, 'TPL': False}, {'DB': True, 'TPL': False}, {'DB': False, 'TPL': True}, {'DB': True, 'TPL': True}]:
            ...
            while i < request_iterations:
                ...
                self.browser.get("{0}{1}".format(angular_domain, '#'+site1_w_caching))
                time.sleep(self.request_timeout)
                ...
                self.browser.get("{0}{1}".format(lare_domain, site2_w_caching))
                time.sleep(self.request_timeout)
                ...
                self.browser.get("{0}{1}".format(lare_domain, site1_w_caching))
                time.sleep(self.request_timeout)
                ...
                self.browser.close()
                self.browser.quit()
            avg_response_time_normal_lare = avg_response_time_normal_lare / request_iterations
            avg_response_time_normal_angular = avg_response_time_normal_angular / request_iterations
            avg_response_time_lare = avg_response_time_lare / request_iterations
            avg_response_time_angular = avg_response_time_angular / request_iterations
            avg_request_count_normal_lare = avg_request_count_normal_lare / request_iterations
            avg_request_count_normal_angular = avg_request_count_normal_angular / request_iterations
            avg_request_count_lare = avg_request_count_lare / request_iterations
            avg_request_count_angular = avg_request_count_angular / request_iterations
            avg_waiting_time_normal_lare = avg_waiting_time_normal_lare / request_iterations
            avg_waiting_time_normal_angular = avg_waiting_time_normal_angular / request_iterations
            avg_waiting_time_lare = avg_waiting_time_lare / request_iterations
            avg_waiting_time_angular = avg_waiting_time_angular / request_iterations
...
if __name__ == '__main__':
    ...
    print args
    if len(args) > 1:
        lare_domain = str(args[1])
    if len(args) > 2:
        angular_domain = str(args[2])
    if len(args) > 3:
        request_iterations = int(args[3])
    unittest.main()